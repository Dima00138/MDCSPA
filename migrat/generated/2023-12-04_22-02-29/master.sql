...
spool "migrat_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_migrat ...
DROP ROLE ROLE_migrat ;
PROMPT Creating Role ROLE_migrat ...
CREATE ROLE ROLE_migrat ;

-- PROMPT Drop Univ user
-- drop user Univ cascade;
   
PROMPT Create user Univ
CREATE USER Univ IDENTIFIED BY &&Univ_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO Univ;
...
GRANT ALL ON utils TO ROLE_migrat;
grant ALL on utils to Univ with grant option;

set define on
prompt connecting to Univ
...
PROMPT Creating Primary Key Constraint PK__PERSONAL__3214EC276C5D076C on table PERSONAL ... 
ALTER TABLE PERSONAL
ADD CONSTRAINT PK__PERSONAL__3214EC276C5D076C PRIMARY KEY
(
  ID
)
ENABLE
...
GRANT ALL ON PERSONAL TO ROLE_migrat;
...
COMMENT ON COLUMN SALARY.DATE_ IS 'ORIGINAL NAME:DATE'
;

PROMPT Creating Primary Key Constraint PK__SALARY__3214EC275B5ABA65 on table SALARY ... 
ALTER TABLE SALARY
ADD CONSTRAINT PK__SALARY__3214EC275B5ABA65 PRIMARY KEY
(
  ID
)
ENABLE
...
GRANT ALL ON SALARY TO ROLE_migrat;
-- DROP TABLE Statuse CASCADE CONSTRAINTS;


PROMPT Creating Table Statuse ...
...
PROMPT Creating Primary Key Constraint PK__Statuse__3213E83F03159B51 on table Statuse ... 
ALTER TABLE Statuse
ADD CONSTRAINT PK__Statuse__3213E83F03159B51 PRIMARY KEY
(
  id
)
ENABLE
...
GRANT ALL ON Statuse TO ROLE_migrat;
-- DROP TABLE Periodes CASCADE CONSTRAINTS;


PROMPT Creating Table Periodes ...
CREATE TABLE Periodes (
  id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  date_beg VARCHAR2(200 CHAR),
  date_end VARCHAR2(200 CHAR)
);


PROMPT Creating Primary Key Constraint PK__Periodes__3213E83F448BCA0B on table Periodes ... 
ALTER TABLE Periodes
ADD CONSTRAINT PK__Periodes__3213E83F448BCA0B PRIMARY KEY
(
  id
)
ENABLE
...
GRANT ALL ON Periodes TO ROLE_migrat;
...
COMMENT ON COLUMN Task.desc_ IS 'ORIGINAL NAME:desc'
;

PROMPT Creating Primary Key Constraint PK__Task__3213E83F2FDB9C0B on table Task ... 
ALTER TABLE Task
ADD CONSTRAINT PK__Task__3213E83F2FDB9C0B PRIMARY KEY
(
  id
)
ENABLE
...
GRANT ALL ON Task TO ROLE_migrat;
-- DROP TABLE TaskStep CASCADE CONSTRAINTS;


PROMPT Creating Table TaskStep ...
...
COMMENT ON COLUMN TaskStep.desc_ IS 'ORIGINAL NAME:desc'
;

PROMPT Creating Primary Key Constraint PK__TaskStep__3213E83FF8742843 on table TaskStep ... 
ALTER TABLE TaskStep
ADD CONSTRAINT PK__TaskStep__3213E83FF8742843 PRIMARY KEY
(
  id
)
ENABLE
...
GRANT ALL ON TaskStep TO ROLE_migrat;
-- DROP TABLE CompleteList CASCADE CONSTRAINTS;


PROMPT Creating Table CompleteList ...
...
COMMENT ON COLUMN CompleteList.desc_ IS 'ORIGINAL NAME:desc'
;

PROMPT Creating Primary Key Constraint PK__Complete__3213E83FB92BEFAC on table CompleteList ... 
ALTER TABLE CompleteList
ADD CONSTRAINT PK__Complete__3213E83FB92BEFAC PRIMARY KEY
(
  id
)
ENABLE
...
GRANT ALL ON CompleteList TO ROLE_migrat;
...
alter session set current_schema=Univ;
set define off
CREATE OR REPLACE FORCE VIEW TASKS_VIEW
...
AS 
   SELECT Задачи.id ,
          ...
          Задачи.описание ,
          Периоды.дата_начала ,
          Периоды.дата_окончания 
     FROM Задачи 
            JOIN Периоды    ON Задачи.период_id = Периоды.id;


GRANT ALL ON TASKS_VIEW TO ROLE_migrat;
CREATE OR REPLACE FORCE VIEW COMPLETED_TASKS_VIEW 
AS 
   SELECT Задачи.название ,
          Задачи.описание ,
          Журнал_выполнения.дата ,
          Статусы_выполнения.описание статус_задачи  
     FROM Журнал_выполнения 
            JOIN Задачи    ON Журнал_выполнения.задача_id = Задачи.id
            JOIN Статусы_выполнения    ON Задачи.статус_задачи_id = Статусы_выполнения.id;


GRANT ALL ON COMPLETED_TASKS_VIEW TO ROLE_migrat;
CREATE OR REPLACE FUNCTION COUNT_COMPLETED_STEPS
(
  v_задача_id IN NUMBER
)
RETURN NUMBER
AS
   v_количество NUMBER(10,0);
...
BEGIN
   SELECT COUNT(*)  
     ...
     INTO v_количество
     FROM Журнал_выполнения 
    WHERE  задача_id = v_задача_id;
   RETURN v_количество;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON COUNT_COMPLETED_STEPS TO ROLE_migrat;
...
BEGIN
   SELECT описание 
     ...
     INTO v_описание
     FROM Задачи 
    WHERE  id = v_задача_id;
   RETURN v_описание;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON GET_TASK_DESCRIPTION TO ROLE_migrat;
CREATE OR REPLACE PROCEDURE AddSubordinateNode
(
  v_NewNodeValue IN UDT.HIERARCHYID%TYPE
)
AS
   -- Найти родительский узел
   v_ParentNode NUMBER(10,0);
...
BEGIN
   ...
   SELECT ParentID 
     ...
     INTO v_ParentNode
     FROM tt_TempTable 
    WHERE  GetAncestor(1) = v_NewNodeValue;
   -- Вставить новый узел с указанием родителя
   INSERT INTO tt_TempTable
     ( ID, ParentID, HierarchyColumn, NAME )
     ( SELECT ( SELECT MAX(ID)  + 1 
                FROM tt_TempTable 
                       ) ,-- Предполагается, что ID является автоинкрементным

              v_ParentNode ,
              v_NewNodeValue ,
              'New Node' -- Здесь можно указать нужное имя для нового узла
...
END;
/
GRANT ALL ON AddSubordinateNode TO ROLE_migrat;
CREATE OR REPLACE PROCEDURE CALCULATE_COUNT_COMPLETE_TASK_STEPS
(
  v_входной_идентификатор_задачи IN NUMBER,
  v_выходное_количество OUT NUMBER
)
AS
...
BEGIN
   ...
   SELECT COUNT(*)  
     ...
     INTO v_выходное_количество
     FROM Журнал_выполнения 
    WHERE  задача_id = v_входной_идентификатор_задачи;
   DBMS_OUTPUT.PUT_LINE('Количество выполненных шагов в задаче ' || UTILS.CONVERT_TO_NVARCHAR2(v_входной_идентификатор_задачи,50) || ': ' || UTILS.CONVERT_TO_NVARCHAR2(v_выходное_количество,50));

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON CALCULATE_COUNT_COMPLETE_TASK_STEPS TO ROLE_migrat;
CREATE OR REPLACE PROCEDURE GetSubordinateNodes
(
  v_NodeValue IN UDT.HIERARCHYID%TYPE
)
AS
...
BEGIN
   ...
   OPEN  v_cursor FOR
      SELECT ToString() NodePath  ,
             GetLevel() NodeLevel  
        FROM tt_TempTable_2 
       WHERE  IsDescendantOf(v_NodeValue) = 1
        ORDER BY NodeLevel,
                 NodePath ;
...
END;
/
GRANT ALL ON GetSubordinateNodes TO ROLE_migrat;
CREATE OR REPLACE PROCEDURE INSERT_TASK
(
  v_название IN NVARCHAR2,
  v_описание IN NVARCHAR2,
  v_период_id IN NUMBER,
  v_статус_задачи_id IN NUMBER
)
AS
...
BEGIN
   ...
   INSERT INTO Задачи
     ( название, описание, период_id, статус_задачи_id )
     VALUES ( v_название, v_описание, v_период_id, v_статус_задачи_id );

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON INSERT_TASK TO ROLE_migrat;
/*Global Temporary Tables:1 *//* Translation Extracted DDL For Required Objects*/
CREATE GLOBAL TEMPORARY TABLE tt_v_SubordinateNodes
(
  HierarchyColumn UDT.HIERARCHYID%TYPE 
);
/
...
(
  v_SourceNodeValue IN UDT.HIERARCHYID%TYPE,
  v_DestinationNodeValue IN UDT.HIERARCHYID%TYPE
)
AS
   v_parent NUMBER(10,0);

BEGIN

   -- Получаем все подчиненные узлы
   INSERT INTO tt_v_SubordinateNodes
     ( SELECT HierarchyColumn 
       FROM tt_TempTable_3 
        WHERE  IsDescendantOf(v_SourceNodeValue) = 1 );
   SELECT ID 

     INTO v_parent
     FROM tt_TempTable_3 
    WHERE  HierarchyColumn = v_DestinationNodeValue;
   UPDATE tt_TempTable_3
      SET ParentID = v_parent
    WHERE  HierarchyColumn = v_SourceNodeValue;
   MERGE INTO sn 
   USING (SELECT sn.ROWID row_id, GetReparentedValue(v_SourceNodeValue, v_DestinationNodeValue) AS HierarchyColumn
   FROM sn ,v_SubordinateNodes sn ) src
   ON ( sn.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET HierarchyColumn = src.HierarchyColumn;
   -- Обновляем значения узлов в таблице
   MERGE INTO tt_TempTable_3 
   USING (SELECT tt_TempTable_3.ROWID row_id, sn.HierarchyColumn
   FROM tt_TempTable_3 ,v_SubordinateNodes sn 
    WHERE tt_TempTable_3.HierarchyColumn = sn.HierarchyColumn) src
   ON ( tt_TempTable_3.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET HierarchyColumn = src.HierarchyColumn;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON MoveSubordinateBranch TO ROLE_migrat;
CREATE OR REPLACE TRIGGER trg_CompleteList_Tasks
   AFTER UPDATE
   ON Task
   FOR EACH ROW
...
BEGIN
   ...
   IF UPDATING('stat_id') THEN
    
   BEGIN
      -- Вставка новой строки в таблицу Журнал_выполнения
      INSERT INTO CompleteList
        ( task_id, step_id, dateE, desc_ )
        ( SELECT :NEW.id ,
                 -1 шаг_id  ,
                 SYSDATE дата  ,
                 'Изменен статус задачи' описание  
            FROM DUAL   WHERE  :NEW.id = :OLD.id );
...
CREATE OR REPLACE TRIGGER trg_CompleteList_Task_Steps
   AFTER UPDATE
   ON TaskStep
   FOR EACH ROW
   ...
   BEGIN
      ...
      INSERT INTO CompleteList
        ...
        ( SELECT :NEW.task_id ,
                 :NEW.id ,
                 SYSDATE дата  ,
                 'Изменен статус шага задачи' описание  
            FROM DUAL   WHERE  :NEW.id = :OLD.id );
...
alter session set current_schema=Univ;
set define off
PROMPT Creating Foreign Key Constraint FK__TaskStep__stat_i__498EEC8D on table Statuse...
ALTER TABLE TaskStep
ADD CONSTRAINT FK__TaskStep__stat_i__498EEC8D FOREIGN KEY
(
  stat_id
)
REFERENCES Statuse
...
PROMPT Creating Foreign Key Constraint FK__TaskStep__task_i__4B7734FF on table Task...
ALTER TABLE TaskStep
ADD CONSTRAINT FK__TaskStep__task_i__4B7734FF FOREIGN KEY
(
  task_id
)
REFERENCES Task
...
PROMPT Creating Foreign Key Constraint FK__SALARY__PERSON__2CF2ADDF on table PERSONAL...
ALTER TABLE SALARY
ADD CONSTRAINT FK__SALARY__PERSON__2CF2ADDF FOREIGN KEY
(
  PERSON
)
REFERENCES PERSONAL
...
PROMPT Creating Foreign Key Constraint FK__Task__stat_id__45BE5BA9 on table Statuse...
ALTER TABLE Task
ADD CONSTRAINT FK__Task__stat_id__45BE5BA9 FOREIGN KEY
(
  stat_id
)
REFERENCES Statuse
...
PROMPT Creating Foreign Key Constraint FK__CompleteL__task___4E53A1AA on table Task...
ALTER TABLE CompleteList
ADD CONSTRAINT FK__CompleteL__task___4E53A1AA FOREIGN KEY
(
  task_id
)
REFERENCES Task
...
PROMPT Creating Foreign Key Constraint FK__Task__per_id__46B27FE2 on table Periodes...
ALTER TABLE Task
ADD CONSTRAINT FK__Task__per_id__46B27FE2 FOREIGN KEY
(
  per_id
)
REFERENCES Periodes
...
PROMPT Creating Foreign Key Constraint FK__CompleteL__step___4F47C5E3 on table TaskStep...
ALTER TABLE CompleteList
ADD CONSTRAINT FK__CompleteL__step___4F47C5E3 FOREIGN KEY
(
  step_id
)
REFERENCES TaskStep
...
PROMPT Creating Foreign Key Constraint FK__TaskStep__per_id__4A8310C6 on table Periodes...
ALTER TABLE TaskStep
ADD CONSTRAINT FK__TaskStep__per_id__4A8310C6 FOREIGN KEY
(
  per_id
)
REFERENCES Periodes
...
ALTER VIEW TASKS_VIEW COMPILE;

ALTER VIEW COMPLETED_TASKS_VIEW COMPILE;
